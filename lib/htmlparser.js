...
exports.HtmlParser = Parser = function(config) {
    this.init(config)
}
...
Parser.prototype = {
    init: function(config) {
        this.processHtml = ''
        this.inputHtml = config.html
        this.reporter = config.reporter
        this.preprocess(config.preprocessor)
        this.isReport = config.isReport
    },
            ...
            if (tagProp.isEndTag) {
                ...
                while (openedNode = stack.last()) {
                    ...
                    // deal with non-close tag
                        ...
                        this.isReport && reporter.error('缺少结束标签</' + openedNode.tagName + '>', lineNo, null, {'id': 'html-tag-close', 'description':'标签应闭合'})
                        stack.pop()
                    }
                }
            } 
