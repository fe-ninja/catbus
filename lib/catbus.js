...
var ignoreList = []
var ruleSet, timeEnd, debug, logfile

exports.scan = function (cmds) {
    var target = cmds.args[0]
    var config = {}
    var configPath, isConfigExist 
    var requireList = []
    var file, fileContent

    // load user config
    configPath = process.cwd() + '/catbus-config.js'
    isConfigExist = fs.existsSync(configPath)
    if (isConfigExist) {
        config = require(configPath).config
        ignoreList = config.ignore
        requireList = config.require || []
    }
    ...
    // load require rules
    cmds.require && requireList.push(cmds.require)

    requireList.forEach(function(requireItem) {
        try {
            var rule = require(requireItem).rule
            ruleSet.rules.push(rule)
            ruleSet.ruleStatus[rule.id] = true
        } 
        catch (e) {
            console.log('fail to require rule module: ' + requireItem)
        }
    })

    if (cmds.list) {
        if (ruleSet.rules && ruleSet.ruleStatus) {
            listRules(ruleSet.ruleStatus)
        }
        process.exit()
    }
...
function fileHandler(error, data, target) {
    ...
    if (ignoreList.length > 0 &&
            new RegExp(ignoreList.join('|')).test(target)) {
        return
    }
    ...
    reporter.log('Current File: ' + target)
    reporter.log('ignore list regexp: ', new RegExp(ignoreList.join('|')))
    ...
    if (env.fileExt === 'js') {
        ...
        htmlParser = new HtmlParser({
            ...
            preprocessor: null,
            reporter: reporter,
            isReport: ruleSet.ruleStatus && ruleSet.ruleStatus['html-tag-close']
        })
        context = htmlParser.parse().document
    }

    if (ruleSet.rules && ruleSet.rules.length > 0) {
        Validator.validate(data, ruleSet.rules, reporter, context)
    }
...
function readDir(dirname, recursive, callback) {
    ...
    files.forEach(function(file) {
        ...
        // in case file name contains Chinese word
        // https://github.com/totorojs/catbus/issues/30
