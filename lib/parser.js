...
function Parser(html, preprocessor) {
    ...
    while (html.length) {
        ...
        // console.log('The Tag Start Position:' + tagResult.index)
        // console.log('The Tag End Position:' + reTag.lastIndex)
        ...
        if (tagProp.isEndTag) {
            ...
            while (true) {
                ...
                if (!_attr) break;
                if (_attr[0] === tagProp.node.tagName) continue;

                // attr数据结构：['class="fn-clear bankAlphabet-list"', 'class', 'fn-clear bankAlphabet-list']
                attr[0] = _attr[0];
                // console.log(attr[0])
    ...
    // 这种情况一般不会出现，此处留待测试
    if (stack.length > 0) {
        console.log('Conclusion: Tag not closed');
    }
