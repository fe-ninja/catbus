...
Parser.prototype = {
    ...
    parse: function(reporter, env, html) {
        ...
        while (tag = reTag.exec(html)) {
            ...
            if (tagProp.isEndTag) {
                ...
                while (stack.length > 0 &&
                        lower(stack.last().tagName) !== lower(tagProp.tagName)) {

                    // 特殊标签未闭合忽略，解析完成最后处理
                    if (lower(stack.last().tagName) in specialTags) {
                        break;
                    }

                    reporter.error('Tag <' + stack.last().tagName + '> not closed.', stack.last().line, null, {'id': 'Tag-closed', 'description':'标签应闭合'}, null);
                ...
                if (stack.last() && lower(stack.last().tagName) in specialTags) {
                    log('[Catbus HtmlParser]: Tag <' + tagProp.tagName + '> in special tag: <' + stack.last().tagName+ '>.')
                    continue;
                }
        ...
        // print the node tree
        var cache = [];
        var str = JSON.stringify(retDom.root, function(key, value) {
