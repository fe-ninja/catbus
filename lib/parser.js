    ...
    while (tagResult = reTag.exec(html)) {
        ...
        while (currentLineBreak = reLine.exec(html)) {
            ...
            if (reLine.lastIndex >= tagResult.index) {
                ...
                currentLine++;
                log('Line ' + currentLine + ' :' + html.substring(reLine.lastIndex, tagResult.index))
            }
            lastLinePos = reLine.lastIndex;
        }

        log('The Tag Start Position:' + tagResult.index)
        log('The Tag End Position:' + reTag.lastIndex)
        ...
        if (tagProp.isEndTag) {
            ...
            while (stack.length > 0 &&
                ...
                console.error('ERROR: Line ' + stack[stack.length - 1].line + ', Tag ' + stack[stack.length - 1].startTag + ' not closed');
