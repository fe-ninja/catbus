...
function normalize(html) {
    ...
    // 去除HTML注释
    html = html.replace(reComment, function(substr){
...
Parser.prototype.parse = function(reporter, env, html) {
    ...
    while (tagResult = reTag.exec(html)) {
        ...
        while (currentLineBreak = reLine.exec(html)) {
            ...
            if (reLine.lastIndex > tagResult.index) {
                ...
                // log('prevLine ' + currentLine + ' :' + html.substring(lastLinePos, lastLinePos + 10) + '...')
                ...
                // log('Line ' + currentLine + ' start at ' + currentLPos + ': ' + html.substring(reLine.lastIndex, reLine.lastIndex + 10) + '...')
                log('reTag lastIndex at ' + reTag.lastIndex + ' tag found: <' + tagResult[1] + tagResult[2] + '>')
            }
        ...
        log('[Catbus HtmlParser]: Tag <' + (tagProp.isEndTag? '/':'') + tagResult[2] + '> start at ' + tagResult.index + ', end at ' + reTag.lastIndex)
        ...
        if (tagProp.isEndTag) {
            ...
            // 特殊标签未闭合
            if (stack.last() && 
                    stack.last().node.tagName.toLowerCase() in special && 
                        stack.last().node.tagName.toLowerCase() !== tagProp.tagName.toLowerCase()) {
                log('[Catbus HtmlParser]: Tag </' + tagProp.tagName + '> in special tag: <' + stack.last() .node.tagName+ '>.')
                continue;
            }
    ...
    if (stack.last() && stack.last().node.tagName in special) {
        console.log('Fatal Error: Line ' + stack.last().line + '. Parsed Error, ' + stack.last().node.tagName + ' tag not closed.');
        process.exit();
    }
